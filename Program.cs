...
namespace SoccerStats
{
    class Program
    {
            ...
            foreach (var player in topTenPlayers)
            {
                ...
                foreach (var sentiment in sentimentResponse.Sentiments)
                {
                    foreach(var newsResult in newsResults)
                    {
                        ...
                        {
                            double score;
                            if(double.TryParse(Detectedlanguage.Score, NumberStyles.Any, CultureInfo.GetCultureInfo("en-US"), out score))
                            {
        ...
        public static List<GameResult> ReadSoccerResults(string fileName)
        {
            ...
            using (var reader = new StreamReader(fileName))
            {
                ...
                while((line = reader.ReadLine()) != null)
                {
                    ...
                    if (DateTime.TryParse(values[0], MyCultureInfo, DateTimeStyles.AdjustToUniversal, out gameDate))
                    {
                    ...
                    {
                        gameResult.GoalsAttempts = parseInt;
                    }
                    ...
                    if (double.TryParse(values[7],NumberStyles.AllowDecimalPoint, MyCultureInfo, out possessionPercent))
                    {
        ...
        public static List<NewsResult> GetNewsForPlayer(string playerName)
        {
            ...
            var webClient = new WebClient();
            webClient.Headers.Add("Ocp-Apim-Subscription-Key", "b9b16ae2dc334e6db139f47344903dda");
        ...
        public static SentimentResponse GetSentimentResponse(List<NewsResult> newsResults)
        {
            ...
            var webClient = new WebClient();
            webClient.Headers.Add("Ocp-Apim-Subscription-Key", "4e679f7f831b420bb17e1b305dd80637");
            webClient.Headers.Add(HttpRequestHeader.Accept, "application/json");
            webClient.Headers.Add(HttpRequestHeader.ContentType, "application/json");
            string requestJson = JsonConvert.SerializeObject(sentimentRequest);
            byte[] requestBytes = Encoding.UTF8.GetBytes(requestJson);
            byte[] response = webClient.UploadData("https://westus.api.cognitive.microsoft.com/text/analytics/v2.0/languages", requestBytes);
